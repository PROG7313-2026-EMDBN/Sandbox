...
fun FocusLogScreen(
    ...
    if (showClearDialog) {
        AlertDialog(
            ...
            title = { Text("Clear today's sessions?") },
            text = { Text("This will delete all focus sessions for ${state.todayLabel}.") },
            confirmButton = {
                Button(
                    onClick = {
                        vm.deleteAllForToday()
    ...
    Column(
        ...
        if (state.isLoading) {
            ...
            Text("No sessions logged for today.")
        } else {
            LazyColumn(
